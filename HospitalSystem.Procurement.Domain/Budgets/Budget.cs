...
public sealed class Budget : AggregateRoot<BudgetId>
{
    ...
    public Money SpentAmount
        => _expenses.Count == 0
            ? Money.Zero(AllocatedAmount.Currency): _expenses
                .Select(x => x.Amount).Aggregate(Money.Zero(AllocatedAmount.Currency),
                    (sum, amount) => sum.Add(amount));

    public Money RemainingAmount
        => SpentAmount.IsGreaterThanOrEqualTo(AllocatedAmount)
        ...
        DateTime incurredOnUtc)
    {
        ...
        {
            AddDomainEvent(new BudgetOverspentDomainEvent(
                    Id,DepartmentId,
                    spent, AllocatedAmount));
        }
    }
}
